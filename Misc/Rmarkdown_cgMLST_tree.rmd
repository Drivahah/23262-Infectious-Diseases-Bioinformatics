...
data <- read.table("cgMLST_table.txt", sep = "\t", row.names=1, colClasses = "factor", header = T)
cgMLST_tree <- as.phylo(hclust(daisy(data, metric="gower")))
write.tree(phy=cgMLST_tree, file="cgMLST_tree.newick")
...
m
m2
write.table(m, 'table_dissimilarity_cgMLST.txt', sep='\t')
```
